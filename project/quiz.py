...
def quizpage():
    ...
    if request.method == "POST":
        if request.form.get('back') or request.form.get('next'): # posted from question advance buttons
            ...
            # return render_template('quiz.html', quiz=Quiz, num=questionNum, curQues=item)
            # global Quiz
            print(Quiz)
            ...
            # global Quiz
            Quiz = quizClass((team1_name, team1_count),(team2_name, team2_count),(team3_name, team3_count))
            print(Quiz)
            return render_template("quiz.html", quiz=Quiz, curQues=Question("Hit Next to start quiz", '', '', ''))
